...
use crate::packet::{PacketSender, PacketReceiver};
...
use crate::tunnel::{PeerAddr, TunnelManager};
...
pub async fn run_client(cfg: Arc<Config>) -> Result<()> {
    log::info!(
        "suit client starting | real={} spoof={} peer={}",
        cfg.real_ip,
        cfg.spoofed_ip,
        cfg.peer_real_ip
    );
...
pub async fn run_server(cfg: Arc<Config>, allow_any: bool) -> Result<()> {
    ...
    log::info!(
        "suit server starting | real={} spoof={} peer={}",
        cfg.real_ip,
        cfg.spoofed_ip,
        cfg.peer_real_ip
    );
    ...
    tokio::spawn(async move {
        mgr_cleanup.await_termination().await;
        log::info!("tunnel manager stopped");
    });

    if let Some(pf_cfg) = &cfg.port_forward {
        log::info!("Starting port forwarder");
        let pf_guard = port_forward::start(pf_cfg.clone(), manager.clone(), cfg.role).await?;
        log::info!("Port forwarder started");
        pf_guard.await_termination().await?;
    } else {
...
async fn run_tun_client(cfg: Arc<Config>, manager: Arc<TunnelManager>) -> Result<()> {
    let tun_mtu = if cfg.tun_mtu == 0 { cfg.mtu } else { cfg.tun_mtu.min(cfg.mtu) };
    if cfg.tun_mtu > cfg.mtu {
        log::warn!("tun_mtu {} > mtu {} - clamping", cfg.tun_mtu, cfg.mtu);
    }
    ...
    let (net_to_tun_tx, net_to_tun_rx): (mpsc::Sender<Bytes>, mpsc::Receiver<Bytes>) = mpsc::bounded(cfg.io_channel_capacity);
    spawn_tun_writer(tun.clone(), net_to_tun_rx);

    let tun_reader = tun.clone();
    let mgr_tun = manager.clone();
    tokio::spawn(async move {
        if let Err(e) = run_tun_reader(tun_reader, mgr_tun, &[cfg.tun_peer_ip]).await {
            log::warn!("tun reader stopped: {}", e);
        }
    });
    ...
    tokio::spawn(async move {
        loop {
            tokio::time::sleep(Duration::from_secs(cfg2.tunnel_establish_timeout_secs)).await;
            if let Err(e) = mgr2.open_tunnel(net_to_tun_tx.clone()).await {
                log::warn!("open_tunnel: {}", e);
            }
        }
    });
